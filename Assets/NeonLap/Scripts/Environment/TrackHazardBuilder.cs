...
namespace NeonLap.Environment
{
    public class TrackHazardBuilder : MonoBehaviour
    {
            ...
            Material carAccent)
        {
            ...
            var density = GameQualitySettings.Preset.HazardDensity;
            if (density <= 0.01f)
                return;
            ...
            var hazardIndices = PickHazardIndices(waypoints.Count, startSkip, density);

            SpawnStaticHazards(waypoints, hazardIndices, trackWidth);
            SpawnMovingHazards(waypoints, startSkip, trackWidth, density);
        }
        ...
        void SpawnMovingHazards(IReadOnlyList<Transform> waypoints, int startSkip, float trackWidth, float density)
        {
            ...
            if (density >= 0.65f)
            {
                var barrelIndex = (startSkip + count * 4 / 5) % count;
                var barrelCenter = ObstaclePhysics.SnapToTrackSurface(
                    GetTrackPoint(waypoints, barrelIndex, trackWidth * 0.15f), 0.9f);
                var barrelAxis = GetForward(waypoints, barrelIndex);
                SpawnRollingBarrel(barrelCenter, barrelAxis, 7f, 1.8f);
            }
        }

        void SpawnBarrel(Vector3 position, Quaternion rotation)
        {
            CreateHazardPrimitive(PrimitiveType.Cylinder, "HazardBarrel",
                ObstaclePhysics.SnapToTrackSurface(position, 0.9f),
                new Vector3(0.75f, 0.9f, 0.75f), rotation, barrelMat);
        }

        void SpawnCrateStack(Vector3 position, Quaternion rotation)
        {
            CreateHazardPrimitive(PrimitiveType.Cube, "CrateA",
                ObstaclePhysics.SnapToTrackSurface(position, 0.45f),
                new Vector3(1.1f, 0.9f, 1.1f), rotation, crateMat);
            CreateHazardPrimitive(PrimitiveType.Cube, "CrateB",
                ObstaclePhysics.SnapToTrackSurface(position + rotation * Vector3.right * 0.15f, 0.95f),
                new Vector3(0.85f, 0.75f, 0.85f), rotation, crateMat);
        }
        ...
        void SpawnConeCluster(Vector3 position, Quaternion rotation, System.Random random)
        {
            for (var i = 0; i < 3; i++)
            {
                ...
                CreateHazardPrimitive(PrimitiveType.Cylinder, "Cone_" + i,
                    ObstaclePhysics.SnapToTrackSurface(position + offset, 0.55f),
                    new Vector3(0.55f, 0.55f, 0.55f), rotation, coneMat);
            }
        }
        ...
        void SpawnDebrisChunk(Vector3 position, Quaternion rotation, System.Random random)
        {
            ...
            CreateHazardPrimitive(PrimitiveType.Cube, "NeonDebris",
                ObstaclePhysics.SnapToTrackSurface(position, 0.35f),
                new Vector3(1.4f, 0.7f, 1.1f), chunkRotation, debrisMat);
        }
        ...
        void SpawnPatrolCar(Vector3 start, Vector3 end, float speed, int colorIndex)
        {
            ...
            HazardVisibilityMarker.Attach(car);
        }
        ...
        void SpawnRollingBarrel(Vector3 center, Vector3 moveAxis, float distance, float speed)
        {
            ...
            barrel.AddComponent<MovingTrackObstacle>()
                .Configure(center, moveAxis, distance, speed, 0.5f);
        }
